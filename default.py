...
class MyPlayer(xbmc.Player):
    ...
    def onPlayBackStarted(self):
        if self.isPlayingVideo() and self.playingvideo is not True:
...
class Hue:
    ...
    def __init__(self, settings, args):
        ...
        if self.params == {}:
            ...
            self.logger.debuglog("unimplemented action call: %s" % action)
    ...
    def test_connection(self):
        ip = self.settings.bridge_ip
        user = self.settings.bridge_user
        r = requests.get('http://%s/api/%s/config' % (ip, user))
    ...
    def most_used_spectrum(self, spectrum, saturation, value, size, overall_value):
        ...
        for i in range(360):
            ...
            #shift index to the right so that groups are centered on primary and secondary colors
                ...
                if colorIndex in hsvRatiosDict:
