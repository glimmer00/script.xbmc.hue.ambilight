...
class Light:
    ...
    def get_current_setting(self):
        ...
        r = requests.get("http://%s/api/%s/lights/%s" % (ip, user, self.light))
        j = r.json()
        start = self.start_setting or {}
        self.start_setting = start
        state = j['state']
        start['on'] = state['on']
        start['bri'] = state['bri']
        ...
        if 'hue' in state:
            if 'colormode' in state:
                if not 'colormode' in start:
                    start['colormode'] = state['colormode']
            if 'ct' in state:
                if not 'ct' in start:
                    start['ct'] = state['ct']
    ...
    def set_light(self, data):
        ...
        self.request_url_put("http://%s/api/%s/lights/%s/state" % (ip, user, self.light), data=data)
    ...
    def brighter_light(self):
        ...
        if not self.livingwhite:
            ...
            if self.override_hue:
                ...
                if self.start_setting['colormode'] == 'ct':
                    data += ',"ct":%s' % self.start_setting['ct']
                else:
...
class Group(Light):
    ...
    def get_lights(self):
        try:
            ...
            user = self.bridge_user
            group = self.group_id
            r = requests.get("http://%s/api/%s/groups/%s" % (ip, user, group))
            j = r.json()
        except:
    ...
    def set_light(self, data):
        ...
        user = self.bridge_user
        group = self.group_id
        url = "http://%s/api/%s/groups/%s/action" % (ip, user, group)
        Light.request_url_put(self, url, data=data)
    ...
    def set_light2(self, hue, sat, bri, dur=20):
        ...
        try:
            ...
            group = self.group_id
            url = "http://%s/api/%s/groups/%s/action" % (ip, user, group)
            self.request_url_put(url, data=data)
        except:
            self.logger.debuglog("WARNING: Request fo bridge failed")
            pass
    ...
    def request_url_put(self, url, data):
        try:
            ...
            # probably a timeout
            self.logger.debuglog("WARNING: Request fo bridge failed. %s" % (e))
            pass
